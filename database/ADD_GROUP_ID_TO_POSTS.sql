-- Add group_id column to community_posts to support group-specific posts

-- 1. Add the group_id column if it doesn't exist
ALTER TABLE community_posts 
ADD COLUMN IF NOT EXISTS group_id UUID REFERENCES fellowship_groups(id) ON DELETE CASCADE;

-- 2. Create an index for efficient group post queries
CREATE INDEX IF NOT EXISTS idx_community_posts_group_id 
ON community_posts(group_id) 
WHERE group_id IS NOT NULL;

-- 3. Update the RLS policy to allow members to see group posts
...
-- Create new policy that allows group members to see group posts
...
-- 4. Modify the existing function to filter by group if needed
...
-- 5. Create a function to get posts for a specific group
